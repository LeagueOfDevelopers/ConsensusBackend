...
namespace ConsensusLibrary.UserContext
{
    public class RegistrationFacade : IRegistrationFacade
    {
        ...
        public bool CheckUserExistence(string email, string password)
        {
            ...
            Ensure.Any.IsNotNull(user, nameof(user), opt => opt.WithException(new UserNotFoundException()));

            return _cryptoService.CompareHashWithString(user.Credentials.PasswordHash, password);
        }
